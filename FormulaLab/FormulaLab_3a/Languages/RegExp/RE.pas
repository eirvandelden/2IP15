...
{ This unit defines subclasses of TNode for representing the following
  ...
  Sorts = RE
    Prop = Hole | Meta | Eps | Id | Seq | Dot | Stick | Range | Star | Plus
      Hole    = []
      Meta    = D[name: String]
      Eps     = []
      Id      = D[name: String]
      ...
      N.B. Seq and Range not fully implemented !!
}
...
class function TRE_Hole.GetKind: TNodeKind;
begin
  Result := rkHole;
end;

class function TRE_Hole.GetNrofSons: Integer;
begin
  Result := 0;
end;

function TRE_Hole.GetSon(I: Integer): TNode;
begin
  raise EGetSonIndex.Create('Node error: Hole.GetSon with index ' + IntToStr(I));
end;

class function TRE_Hole.IsHole: Boolean;
begin
  Result := true;
end;

class function TRE_Hole.OpName: String;
begin
  Result := '??';
end;

procedure TRE_Hole.SetSon(I: Integer; ANode: TNode);
begin
...
constructor TRE_Meta.Create(AName: String);
begin
  inherited Create;
  FName := AName;
end;

function TRE_Meta.GetData: String;
begin
  Result := FName;
end;

class function TRE_Meta.GetKind: TNodeKind;
begin
  Result := rkMeta;
end;

class function TRE_Meta.GetNrofSons: Integer;
begin
  Result := 0;
end;

function TRE_Meta.GetSon(I: Integer): TNode;
begin
  raise EGetSonIndex.Create('Node error: Meta.GetSon with index ' + IntToStr(I));
end;

class function TRE_Meta.HasData: Boolean;
begin
  Result := true;
end;

class function TRE_Meta.IsMeta: Boolean;
begin
  Result := true;
end;

class function TRE_Meta.OpName: String;
begin
  Result := 'Meta';
end;

procedure TRE_Meta.SetData(AString: String);
begin
  FName := AString;
end;

procedure TRE_Meta.SetSon(I: Integer; ANode: TNode);
begin
...
constructor TRE_Id.Create(AName: String);
begin
  inherited Create;
  FName := Aname;
end;
...
procedure TRE_Id.SetData(AString: String);
begin
  FName := AString;
end;
...
initialization //===============================================================
  ...
  VREFactory := TSimpleNodefactory.Create;
  with VREFactory do
  begin
    AddNodeClass(TRE_Hole);
    AddNodeClass(TRE_Meta);
    AddNodeClass(TRE_Eps);
    AddNodeClass(TRE_Id);
    AddNodeClass(TRE_Dot);
    AddNodeClass(TRE_Stick);
    AddNodeClass(TRE_Star);
    AddNodeClass(TRE_Plus);
    AddNodeClass(TRE_Range);
    AddNodeClass(TRE_Seq);
  end;
