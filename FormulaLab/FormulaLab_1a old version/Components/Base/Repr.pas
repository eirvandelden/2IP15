...
// Replace the follwing line by your own code
  ...
  Assert( ANode.HasData, 'TRepr.TreeToString.pre failed; no data');

  {
  case ANode.GetNrofSons of
    0: begin
         Rep(ANode, 0); // return first sign

         ANode.
       end; // end 0

    1: begin
        TreeToString( ANode.GetSon(0));     // Recurse on single node
       end; // end 1

    2: begin
        TreeToString( ANode.GetSon(0) );    // Recurse Left
        TreeToString( ANode.GetSon(1) );    // Recurse Right
       end; // end 2
